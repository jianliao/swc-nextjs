...
export function generateSource(elements, fileImports) {
...
${elements.reduce((pre, element) => pre + `export const Sp${element.name} = createComponent(React, '${element.tagName}', ${element.name}, {}, 'Sp${element.name}');\n`, '')}
`
...
${elements.reduce((pre, element) => pre + `export const Sp${element.name} = dynamic<${element.name} | { children?: ReactNode }>(() => import('./${elements[0].name}').then(m => m.Sp${element.name} as any), { ssr });` + '\n', '')}
`;
...
function run(component) {
  ...
  const { indexSource, componentSource } = generateSource(elements, fileImports);
