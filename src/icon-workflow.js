...
import { ${displayName} } from '@spectrum-web-components/icons-workflow/src/elements/${displayName}.js';
...
const generate = async () => {
    glob(path.resolve(__dirname, '..', 'node_modules/@spectrum-web-components/icons-workflow/icons/**.js'), async (error, icons) => {
        ...
        for (let icon of icons) {
            ...
            await outputFile(path.resolve(__dirname, '..', `components/icons-workflow/${displayName}/${displayName}.ts`), fileContent);

            fileContent = generateIndex(elementName, displayName);
            await outputFile(path.resolve(__dirname, '..', `components/icons-workflow/${displayName}/index.ts`), fileContent);
            rootIndexContent = rootIndexContent.concat(`export * from './${displayName}';\n`);
        }
        await outputFile(path.resolve(__dirname, '..', `components/icons-workflow/index.ts`), rootIndexContent);
    });
}
