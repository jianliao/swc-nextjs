...
const generate = async () => {
    glob(path.resolve(__dirname, '..', 'node_modules/@spectrum-web-components/icons-ui/icons/**.js'), async (error, icons) => {
        ...
        for (let icon of icons) {
            ...
            await outputFile(path.resolve(__dirname, '..', `components/icons-ui/${displayName}/${displayName}.ts`), fileContent);

            fileContent = generateIndex(elementName, displayName);
            await outputFile(path.resolve(__dirname, '..', `components/icons-ui/${displayName}/index.ts`), fileContent);
            rootIndexContent = rootIndexContent.concat(`export * from './${displayName}';\n`);
        }
        await outputFile(path.resolve(__dirname, '..', `components/icons-ui/index.ts`), rootIndexContent);
    });
}
